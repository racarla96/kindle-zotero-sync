...
- **Pendiente:** seguimos sin SSH configurado a este Kindle en la sesión. Si el crash persiste tras estos cambios, el siguiente paso ineludible es sacar el traceback real de `koreader.log` (ver README § Troubleshooting) — sin eso, cualquier fix adicional sigue siendo una conjetura basada en lectura de código, no en evidencia del fallo real.
