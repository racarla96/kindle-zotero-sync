...
- ✅ **Confirmed on-device (partial):** metadata sync and Browse library's item listing both work on the real Kindle. ✅ **Fixed from real on-device symptoms** (not guesses): the "Downloads" crash, and downloads/"Test connection" hanging forever with no error — see Known gaps below for both. Neither fix has been re-tested on-device yet.
...
- ✅ **Fixed: downloads and "Test connection" got stuck forever with no error.** Confirmed on-device: a download would sit on `[downloading…]` indefinitely, and "Test connection" would show only "Testing…" and never follow up. Root cause, found by re-reading `WebDAVClient.lua` against the real `frontend/httpclient.lua` source: every `coroutine.resume(co, ...)` call in `download_attachment`/`test_connection` discarded its `ok, err` return values. Lua's `coroutine.resume` doesn't propagate an error inside the coroutine as a Lua error — it just returns `false, message` to whoever called `resume`. Since nothing checked that, *any* runtime error anywhere in the coroutine body (before or after the yield) was silently swallowed: no log line, `callback(...)` never called, so the UI waited forever with zero indication anything had gone wrong. Fixed by checking `ok, err` on every `coroutine.resume` and calling `callback(false, ...)` on failure so the UI actually reflects it. Also removed the `socketutil:set_timeout`/`reset_timeout` calls that used to wrap these requests — verified against the real source that this client is Turbo-based, not LuaSocket, so those timeouts never applied here at all (Turbo hardcodes its own 10s/20s connect/request timeouts internally instead); it was dead code that implied a safety net that didn't exist. Separately, added the same `NetworkMgr:willRerunWhenOnline()` guard `sync()` already had to `downloadPendingAttachments`, `drainQueue`, and "Test connection" — none of them checked whether Wi-Fi was actually up before firing a request, and Kindle's Wi-Fi radio sleeps aggressively on idle, so a request fired into a dead radio would (at best) wait out Turbo's own timeout with no earlier signal.
