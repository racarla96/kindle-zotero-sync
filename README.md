...
- ⚠️ **Confirmed on-device: opening "Downloads" crashed/froze KOReader.** Root cause not yet confirmed via an actual log (see [Troubleshooting](#troubleshooting-could-not-reach-the-zotero-api-or-similar-errors-on-device)), but one definite bug was found and fixed by re-reading `Menu`'s real source: a `separator = true` field on one row, which is a valid key for the *main* Zotero menu (`addToMainMenu`'s TouchMenu-style `sub_item_table`) but not for this generic `Menu:new{item_table=...}` widget — it doesn't exist there at all. Also replaced a "↻" glyph in that same row with plain text as a precaution, and wrapped the whole screen in `pcall` so a future error here shows a message instead of crashing outright — **note that the `pcall` only catches errors in the setup code before `UIManager:show()`, not errors during a later async render pass**, so it's a partial mitigation, not a guaranteed fix. If it still crashes, the koreader.log traceback is what's actually needed next.
